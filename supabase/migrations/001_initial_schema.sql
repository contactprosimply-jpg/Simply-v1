-- Simply — Schéma standalone (projet Supabase dédié Simply)
-- À exécuter sur le projet Supabase Simply UNIQUEMENT (pas Operis).
-- La connexion Operis ↔ Simply viendra plus tard.
...
CREATE POLICY organisations_select_member ON public.organisations FOR SELECT
  USING (id = public.current_organisation_id());

DROP POLICY IF EXISTS chantiers_select_org ON public.chantiers;
CREATE POLICY chantiers_select_org ON public.chantiers FOR SELECT
  USING (organisation_id = public.current_organisation_id() OR owner_id = auth.uid());

DROP POLICY IF EXISTS chantiers_insert_org ON public.chantiers;
CREATE POLICY chantiers_insert_org ON public.chantiers FOR INSERT
  WITH CHECK (
    owner_id = auth.uid()
    AND organisation_id = public.current_organisation_id()
  );

DROP POLICY IF EXISTS chantiers_update_org ON public.chantiers;
CREATE POLICY chantiers_update_org ON public.chantiers FOR UPDATE
  USING (organisation_id = public.current_organisation_id() OR owner_id = auth.uid());

DROP POLICY IF EXISTS taches_select ON public.taches;
CREATE POLICY taches_select ON public.taches FOR SELECT USING (public.user_can_access_chantier(chantier_id));
DROP POLICY IF EXISTS taches_insert ON public.taches;
CREATE POLICY taches_insert ON public.taches FOR INSERT WITH CHECK (public.user_can_access_chantier(chantier_id));
DROP POLICY IF EXISTS taches_update ON public.taches;
CREATE POLICY taches_update ON public.taches FOR UPDATE USING (public.user_can_access_chantier(chantier_id));
DROP POLICY IF EXISTS taches_delete ON public.taches;
CREATE POLICY taches_delete ON public.taches FOR DELETE USING (public.user_can_access_chantier(chantier_id));

DROP POLICY IF EXISTS photos_select ON public.photos;
CREATE POLICY photos_select ON public.photos FOR SELECT USING (public.user_can_access_chantier(chantier_id));
DROP POLICY IF EXISTS photos_insert ON public.photos;
CREATE POLICY photos_insert ON public.photos FOR INSERT WITH CHECK (public.user_can_access_chantier(chantier_id));

DROP POLICY IF EXISTS budget_select ON public.budget_lignes;
CREATE POLICY budget_select ON public.budget_lignes FOR SELECT USING (public.user_can_access_chantier(chantier_id));
DROP POLICY IF EXISTS budget_insert ON public.budget_lignes;
CREATE POLICY budget_insert ON public.budget_lignes FOR INSERT WITH CHECK (public.user_can_access_chantier(chantier_id));
DROP POLICY IF EXISTS budget_update ON public.budget_lignes;
CREATE POLICY budget_update ON public.budget_lignes FOR UPDATE USING (public.user_can_access_chantier(chantier_id));

DROP POLICY IF EXISTS plans_select ON public.plans;
CREATE POLICY plans_select ON public.plans FOR SELECT USING (public.user_can_access_chantier(chantier_id));
